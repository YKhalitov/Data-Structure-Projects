...
// Class: CSC 103
// Description: This table works with chained hashing
...
public class TableChainHash< K , E >
{
   ...
   //      element key is placed at keys[i].
   ...
   **/   
   ...
   /**
   * Return the number of collisions occured trying to place data
   * @param - none
   ...
   **/
   ...
   // value is the index of the specified key. Otherwise, the return value is -1.
   {
      ...
      {
         if (keys[i].listSearch(keys[i], key) != null)
            return i;
         count++;
         i = nextIndex(i);
      }
   ...
   **/
   ...
   // calculated as the remainder when the absolute value of the key
   // hash code is divided by the size of the table arrays.
   {
   ...
   *   the new element that being added to this table
   * <dt><b>Precondition:</b><dd>
   *   If there is not already an element with the specified <CODE>key</CODE>,
   *   then this tablesize must be less than its capacity 
   ...
   **/
   public E put(K key, E element)
   {
      ...
      {  // The key is not yet in this Table.
         ...
         //data
         if (data[index] == null){
            keys[index] = new Node(key, null); //data, initial link
            data[index] = new Node(element, null); //data, initial link
         }else{
            keys[index].addNodeAfter(element);
            data[index].addNodeAfter(element);
            collisionCounter = collisionCounter + keys[index].listLength(keys[index]) - 1;
         }
   ...
   **/
   public Node remove(K key)
   {
      ...
      if (index != -1)
      {
         //get reference to node that got removed
         answer = keys[index].listSearch(keys[index], key);
         
         //removal
         if (keys[index].listLength(keys[index]) != 1){
            //preset references
            previousNode = keys[index].listPosition(keys[index], 1);
            nextNode = keys[index].listPosition(keys[index], 2);
             
            //loop through list looking for the node before the right node
            for (int i = 0; i <= keys[index].listLength(keys[index]); i++){      
               if (nextNode.getData() == answer.getData()){
                  previousNode.removeNodeAfter();
               }else{
                  previousNode = nextNode;
                  nextNode = keys[index].listPosition(keys[index], i + 3); 
               }   
